...
/** Applies an attachment transform and submits Anatomica's quad-safe geometry. */
final class AnatomicaAttachmentRenderer {

    private AnatomicaAttachmentRenderer() {
    }

    static <S extends HumanoidRenderState, M extends HumanoidModel<S>> void render(PoseStack poseStack,
            SubmitNodeCollector renderQueue, S state, M model, AttachmentPoint attachment, Mesh baseMesh,
            MeshDeformer deformer, RenderType renderType, int light, int overlay, int color, float partialTick) {
        ...
        if (attachment.localRotationEuler() != null && !attachment.localRotationEuler().equals(Vec3.ZERO)) {
            poseStack.mulPose(new org.joml.Quaternionf().rotationZYX(
                    attachment.localRotationEuler().z(), attachment.localRotationEuler().y(),
        ...
        renderQueue.submitCustomGeometry(poseStack, renderType,
                new AnatomicaGeometryRenderCommand(baseMesh, deformer.deform(baseMesh, partialTick), light, overlay,
                        color));
        poseStack.popPose();
    }
