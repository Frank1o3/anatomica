 ...
 * {@link BodyConfigKey} that validates/(de)serializes it. Disk persistence and
 * network
 * sync both walk {@link #ENTRIES} rather than maintaining their own per-field
 * code, so
 * adding a new customization field means adding one entry here — not touching
 * serialization logic in three different places.
 */
